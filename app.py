...
# ---------- Env & API Key ----------
load_dotenv()
# Supports either OPENAI_API_KEY or OPENROUTER_API_KEY in .env
api_key = os.getenv("OPENAI_API_KEY") or os.getenv("OPENROUTER_API_KEY")

# Optional: GPT API (OpenAI-style)
try:
    import openai
    GPT_AVAILABLE = True
except Exception:
...
"""
    ...
    try:
        # use sidebar key if present, else .env
...
def generate_recommendation():
    ...
    if (st.session_state.use_api or api_key) and GPT_AVAILABLE:
    ...
    allergies = (patient.get("allergies") or "").lower()  # currently unused, but kept
    ...
    am_routine = facepacks_df[
        (facepacks_df.am_pm == "AM")
        & (facepacks_df.suitable_for.str.contains(skin_type, case=False, na=False))
    ]
    ...
    # PM Routine
    lines.append("\n### 🌙 Night Routine")
    pm_routine = facepacks_df[
        (facepacks_df.am_pm == "PM")
        & (facepacks_df.suitable_for.str.contains(skin_type, case=False, na=False))
    ]
...
if GPT_AVAILABLE:
    key_input = st.sidebar.text_input(
        "Optional: Enter OpenAI-compatible API Key for personalized routines",
        type="password",
    )
    if key_input:
    ...
    if not user_input.strip():
        ...
        if stage in ["greet", "concern"]:
            ...
            if detect_severe_keywords(user_input):
                append_message(
                    "assistant",
                    "Red-flag symptoms detected. Consider seeing a doctor. Book appointment?",
                )
                ...
                append_message(
                    "assistant",
                    f"Got it — noted: {', '.join(concerns)}. When did this start?",
                )
                st.session_state.stage = "duration"
            else:
                append_message(
                    "assistant",
                    "Could you specify your main concern (acne/pigmentation/dryness/oiliness/redness)?",
                )
            ...
            append_message(
                "assistant",
                "Thanks — what's your skin type? (dry/oily/combination/sensitive)",
            )
            ...
            append_message(
                "assistant",
                "Nice — tell me your current routine briefly.",
            )
            ...
            append_message(
                "assistant",
                "Any allergies or ingredients you avoid? (or type 'no')",
            )
            ...
            append_message(
                "assistant",
                "Great — crafting your tailored routine 💖",
            )
            ...
            if "yes" in user_input.lower():
                append_message(
                    "assistant",
                    "Please fill the booking form on the right.",
                )
                st.session_state.stage = "booking"
            else:
                append_message(
                    "assistant",
                    "Alright — what's your skin type?",
                )
                st.session_state.stage = "skin_type"
            ...
            append_message(
                "assistant",
                "Would you like to book an appointment or save this consult? (book/save/none)",
            )
            ...
            if "book" in user_input.lower():
                append_message(
                    "assistant",
                    "Please fill the booking form in the sidebar.",
                )
                st.session_state.stage = "booking"
...
uploaded = st.sidebar.file_uploader(
    "Upload face photo (optional)", type=["png", "jpg", "jpeg"]
)
if uploaded:
...
with st.sidebar.form("booking_form"):
    ...
    if submitted:
        ...
        append_message(
            "assistant",
            f"Booked provisional slot for {name or 'Anonymous'} on {date} at {time} in {city}.",
        )

st.caption(
    "SkinSync — advice is non-diagnostic. Seek licensed professional if severe symptoms."
)
